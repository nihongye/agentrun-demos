...
#   -n <name>       镜像名称（默认: demo-code-execution-mcp）
...
#
# 前置条件：
#   1. 已登录目标镜像仓库（docker login ...）
#   2. 已安装 Docker >= 19.03 且启用 buildx 插件
#   3. 本地已执行 npm run build（生成 dist/）
# =============================================================================
...
DEFAULT_NAME="demo-code-execution-mcp"
DEFAULT_TAG="latest"
...
if [ ! -d "${SCRIPT_DIR}/dist" ]; then
    echo "[INFO] dist/ 不存在，正在执行 npm run build..."
